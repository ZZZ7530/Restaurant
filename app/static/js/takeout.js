...
const takeoutMessage = document.querySelector("#takeoutMessage");
...
if (takeoutForm) {
  ...
  takeoutForm.addEventListener("submit", async (event) => {
    ...
    takeoutMessage.textContent = "";

    if (cart.size === 0) {
      takeoutMessage.textContent = "請先加入至少一項餐點。";
      takeoutMessage.className = "form-message text-danger";
      return;
    }
    ...
    try {
      ...
      takeoutMessage.textContent = response.ok ? result.message : (result.error || `外帶訂單送出失敗，HTTP ${response.status}`);
      takeoutMessage.className = response.ok ? "form-message text-success" : "form-message text-danger";
      ...
      takeoutMessage.textContent = `外帶訂單送出失敗：${error.message}`;
      takeoutMessage.className = "form-message text-danger";
    }
  });
}
